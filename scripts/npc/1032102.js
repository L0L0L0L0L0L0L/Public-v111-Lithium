		...
		if (status == 0) {
			...
			if (selection == 0) {
				...
				if (pet == null || !cm.haveItem(5380000,1)) {
					...
					cm.gainPet(after, name, level, closeness, fullness, 90, flag);
				...
				for (var i = 0; i <= inv.getSlotLimit(); i++) {
					...
					if (it != null && it.getItemId() >= 5000000 && it.getItemId() < 5030000 && it.getExpiration() > 0 && it.getExpiration() < cm.getCurrentTime()) {
						theitems.push(it);
			...
			if (theitems.length <= 0) {
				...
				cm.sendOk("All done.. your pet's life has been extended to 90 days from today.");
