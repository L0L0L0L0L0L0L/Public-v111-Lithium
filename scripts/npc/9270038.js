/*
    Name:    Saro
    Map:     Singapore Airport
    Description: 540010000
*/
...
function start() {
	var chat = "Hello, I’m Saro from Singapore Airlines. How can I assist you today? #b";
	chat += "\r\n#L0#Buy a ticket to Fallen City";
	...
	switch (selection) {
		...
		case 1:
			...
			if (!cm.getPlayer().itemQuantity(ticket)) {
				cm.sendOk("You don’t have #v" + ticket + "#, so I can't let you into the waiting room.");
				cm.dispose();
				return;
			}
			if (em.getProperty("entry") == "false" && em.getProperty("docked") == "true") {
				cm.sendOk("Dear customer, please note the flight schedule. Ticket checking stops one minute before departure.");
				cm.dispose();
				return;
			}
