/*
    ...
    Map: Suspicious Laboratory
    Description: 926110000
*/
...
function start() {
	...
	if ((progress >> y) % 2 == 0) {
		if (eim.getProperty("1ch") == y) {
			...
			cm.sendNext("You have found a letter that seems to be addressed to a lover.");
		}

		if (eim.getProperty("2ch") == y) {
			cm.getPlayer().getMap().getReactorByName("d00").forceHitReactor(1);
			cm.getClient().getSession().write(Packages.tools.packet.MaplePacketCreator.getTopMsg("You have opened the entrance to the laboratory"));
