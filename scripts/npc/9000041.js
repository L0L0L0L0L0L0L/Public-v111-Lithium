	...
	switch (status) {
		case 0:
			if (cm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(29503)).getStatus() == 1)
				cm.sendSimple("There is a small note above saying, please donate for village construction! Your kindness will be rewarded.\r\n#L1##bDonate mesos to Maple World#l");
			else
				cm.sendSimple("Hello, did you collect any #b#v4001126##z4001126##k while hunting? I am collecting them.\r\n#L0##bDonate 10 Maple Leaves#l");
			break;
		case 1:
			select = selection;
			if (selection == 0) {
				if (cm.getPlayer().itemQuantity(4001126) < 10) {
					cm.sendOk("#v4001126# Quantity is insufficient, any monster in Maple World might drop them.");
					cm.dispose();
					return;
				}
				...
				if (random < reward.length) {
					if (cm.canHold(reward[random])) {
						...
						cm.sendOk("Congratulations, you have received #v" + reward[random] + "# as a reward.");
						cm.dispose();
						return;
					}
					cm.sendOk("Before receiving the item, please check if there is enough space in your inventory.");
					cm.dispose();
					return;
				}
				cm.gainItem(4001126, -10);
				...
				cm.gainMeso(meso);
				cm.sendOk("Thank you for your support, please accept " + meso + " mesos as encouragement.");
				cm.dispose();
				return;
			}
			if (selection == 1) {
				cm.sendGetNumber("Please enter the amount you wish to donate.", 1, 1, 999999999);
			}
			break;
		case 2:
			...
			cm.sendOk("Thank you for your support of the Maple World.");
			cm.dispose();
			break;
	}
}