...
function action(mode, type, selection) {
    ...
    switch (status) {
        case 0:
            var chat = "Welcome to the Android Beauty Manager. I am Sunny. With a membership card from our shop, you can enjoy our professional services. #b";
            chat += "\r\n#L0##v5150053##t5150053#";
            chat += "\r\n#L1##v5151036##t5151036#";
            chat += "\r\n#L2##v5152057##t5152057#";
            chat += "\r\n#L3##v5152103# Daily Disposable Contact Lenses";
            cm.sendSimple(chat);
            break;
        case 1:
            if (cm.getPlayer().getAndroid() == null) {
                cm.sendOk("Sorry, you must equip an android to use this service!");
                cm.dispose();
                return;
            }
            if (selection == 0) {
                ...
                for (var i = 0; i < hair.length; i++)
                    hair[i] = hair[i] + parseInt(cm.getPlayer().getAndroid().getHair() % 10); // Keep current hair color

                cm.sendAndroidStyle("Using our specialized machine, you can preview your robot's look after the hairstyle. Choose a style you like.", hair);
            }
            if (selection == 1) {
                ...
                cm.sendAndroidStyle("Using our specialized machine, you can preview your robot's look after dyeing. Choose a color you like.", hair);
            }
            if (selection == 2) {
                if (cm.getPlayer().hasEquipped(1662000) || cm.getPlayer().hasEquipped(1662002))
                ...
                cm.sendAndroidStyle("Using our specialized machine, you can preview your robot's look after the facial transformation. Choose a style you like.", face);
            }
            if (selection == 3) {
                ...
                cm.sendAndroidStyle("Using our specialized machine, you can preview your robot's look after applying contact lenses. Choose a lens color you like.", color);
            }
            select = selection;
            break;
        case 2:
            if ((select == 0 && cm.getPlayer().itemQuantity(5150053)) || (select == 1 && cm.getPlayer().itemQuantity(5151036))) {
                ...
                cm.sendOk("Your robot's style has been updated. Do you like it?");
                cm.dispose();
                return;
            }
            ...
            cm.sendOk("Sorry, without the required " + (select == 3 ? "contact lens color" : "membership card") + ", I'm unable to assist you.");
            cm.dispose();
    }
}
