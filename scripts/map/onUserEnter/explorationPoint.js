...
function start() {
	if (ms.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(24046)).getStatus() == 1) {
		...
		ms.getClient().getSession().write(Packages.tools.packet.MaplePacketCreator.getTopMsg("After sleeping for hundreds of years, this place has developed into a new village."));
		}
	...
	if (ch == '0') {
		...
		ms.getClient().getSession().write(Packages.tools.packet.MaplePacketCreator.getTopMsg("" + top +"" + (parseInt(y) + 1) + " / " + (quest == 29005 ? 20 : 10) + "complete"));
		}
		}
		ms.dispose();
}