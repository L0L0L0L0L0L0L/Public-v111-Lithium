...
function action(mode, type, selection) {
	...
	case 0:
		...
		ms.getClient().getSession().write(Packages.tools.packet.EtcPacket.environmentChange("demonSlayer/text11", 3));
		ms.getClient().getSession().write(Packages.tools.packet.EtcPacket.UIPacket.getDirectionInfo(1, 4000));
		break;
	case 1:
		ms.getClient().getSession().write(Packages.tools.packet.EtcPacket.EffectPacket.ShowWZEffect("Effect/Direction6.img/DemonTutorial/Scene2"));
		ms.getClient().getSession().write(Packages.tools.packet.EtcPacket.UIPacket.getDirectionInfo(1, 9000));
		break;
	case 2:
		ms.dispose();
