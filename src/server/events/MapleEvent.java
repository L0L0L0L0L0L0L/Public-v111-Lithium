...
public abstract class MapleEvent {
    ...
    public static void givePrize(final MapleCharacter chr) {
        ...
        if (reward == 0) {
            ...
            chr.dropMessage(5, "[Tip] Congratulations, you have obtained " + mes + "Meso's rewards.");
        } else if (reward == 1) {
            final int cs = Randomizer.nextInt(4000) + 1000;
            chr.modifyCSPoints(1, cs, true);
            chr.dropMessage(5, "[Tip] Congratulations, you have obtained " + cs + "Rewards for NX.");
        } else if (reward == 2) {
            final int cs = Randomizer.nextInt(4000) + 1000;
            chr.modifyCSPoints(1, cs, true);
            chr.dropMessage(5, "[Tip] Congratulations, you have obtained " + cs + "Rewards for NX.");
            ...
            chr.addFame(10);
            chr.dropMessage(5, "[Tip] Congratulations, you have earned a reputation of 10 points");
	} else if (reward == 4) {
	    chr.dropMessage(5, "[Tip] Bang~~There are no rewards.");
        } else {
            int max_quantity = 1;
            switch (reward) {
                case 5062000:
            ...
            chr.dropMessage(5, "[Tip] Congratulations, you have obtained a random item.");
        ...
        if (auto && cserv.getEvent() > -1) {
            for (MapleEventType t : MapleEventType.values()) {
			   ...
			   // World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(0, "Entries for the event are now closed!"));
                           //
                            //e.broadcast(MaplePacketCreator.serverNotice(0, "The activity will start in 30 seconds."));
   ...
   //     World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(0, "Hello!  " + StringUtil.makeEnumHumanReadable(event.name()) + " activity " + cserv.getChannel() + "It's about to start. " + cserv.getChannel() + "Please go to the channel and enter @ event."));
        return "";
    }
}
