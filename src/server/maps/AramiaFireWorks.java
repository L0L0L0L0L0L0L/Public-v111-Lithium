...
public class AramiaFireWorks {
    ...
    private final void broadcastServer(final MapleCharacter c, final int itemid) {
        switch(itemid){
            case SUN_ID:{
                World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, itemid, "After a while, the maple trees on the maple slope were about to explode."));
            }
            case DEC_ID:{
                World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, itemid, "After a while, the happy village and the happy tree are about to snow!"));
            }
            case KEG_ID:{
                World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(5, itemid, "Later, Ami in Heines Park will set off fireworks and firecrackers."));
            }
        }
    }
