...
public class MapleInventoryManipulator {
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (source.getItemId() / 10000 == 166) {
            ...
            if(target2 == null) {
                c.getPlayer().dropMessage(1, "An Android cardiac prop needs to be installed to start.");
                c.getSession().write(MaplePacketCreator.enableActions());
                return;
            }
        ...
        } else if (source.getItemId() == 1122121) { //auto-loot item - petvac
            if (chr.getAutoLoot() == false) {
                chr.setAutoLoot(true);
            }
           chr.dropMessage(6, "Wearing special equipment can activate its attributes.");
        }
            ...
            for (int i : potentials) {
                if (i > 0) {
                    ...
                    if (pot != null && pot.get("skillID") > 0) {
                        chr.dropMessage(5, SkillFactory.getSkillName(pot.get("skillID")) +" Skill activated.");
    ...
    public static void unequip(final MapleClient c, final short src, final short dst) {
        ...
        if(source.getAndroid2()) {
            ...
            if(target2 != null) {
                c.getPlayer().dropMessage(1, "Please remove the Android component first.");
                c.getSession().write(MaplePacketCreator.enableActions());
                return;
            }
        }
        ...
        if (source.getState() >= 17) {
            ...
            for (int i : potentials) {
                if (i > 0) {
                    StructItemOption pot = ii.getPotentialInfo(i).get(ii.getReqLevel(source.getItemId()) / 10);
                    if (pot != null && pot.get("skillID") > 0) {
                        c.getPlayer().dropMessage(5, SkillFactory.getSkillName(pot.get("skillID")) +" The skill is prohibited from being used.");
