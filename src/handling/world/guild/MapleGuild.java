        ...
        for (final MapleGuildCharacter member : members) {
            if (member.getId() == mgc.getId()) {
                ...
                if (old_level != mgc.getLevel()) {
                    ...
                    this.broadcast(MaplePacketCreator.serverNotice(5, "< Guild > " + mgc.getName() + "Upgrade to" + mgc.getLevel() + "Level，Let's congratulate her."));
                }
                if (old_job != mgc.getJobId()) {
                    //this.broadcast(CWvsContext.sendJobup(false, mgc.getJobId(), mgc.getName()), mgc.getId());
                    this.broadcast(MaplePacketCreator.serverNotice(5, "< Guild > " + mgc.getName() + "Promoted to" + GameConstants.getJobName(mgc.getJobId()) + "，Let's congratulate her."));
                }
