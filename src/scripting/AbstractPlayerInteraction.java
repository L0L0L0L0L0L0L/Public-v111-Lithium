        ...
        if (quantity >= 0) {
            ...
            if (type.equals(MapleInventoryType.EQUIP) && !GameConstants.isThrowingStar(id) && !GameConstants.isBullet(id)) {
                ...
                if (id / 10000 == 114 && name != null && name.length() > 0) { //medal
                    final String msg = "<" + name + "> Obtain a title.";
   ...
   public void setKeyValue(String key, String value) {
    ...
    public void warpParty(int id, boolean isNext) {
       ...
       if (mmap.getCharactersSize() > 0) {
           c.getPlayer().dropMessage(1, "Other groups have already entered, please try again later.");
           return;
       }
   ...
   public final int getAndroidStat(final String type) { 
    ...
    public final void startAriantPQ(int mapid) {
        for (final MapleCharacter chr : c.getPlayer().getMap().getCharacters()) {
            ...
            chr.getClient().getSession().write(EtcPacket.getClock(8 * 60));
            ...
            Timer.EtcTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                    chr.getClient().getSession().write(EtcPacket.showAriantScoreBoard());
                    Timer.MapTimer.getInstance().schedule(new Runnable() {
                        @Override
                        public void run() {
