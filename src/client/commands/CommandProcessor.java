...
public class CommandProcessor {
    ...
    public static void help(MapleClient c) {
	final StringBuilder sb = new StringBuilder("Command help: ");
    ...
    public static boolean processCommand(MapleClient c, String line, CommandType type) {
        if (line.charAt(0) == PlayerGMRank.NORMAL.getCommandPrefix() || (c.getPlayer().getGMLevel() > PlayerGMRank.NORMAL.getLevel() && line.charAt(0) == PlayerGMRank.DONATOR.getCommandPrefix())) {
            ...
            if (co == null || co.getType() != type) {
                c.getPlayer().dropMessage(6, "Command " + splitted[0] + " does not exist or you do not have permission to execute it");
                return true;
            }
            try {
                int ret = co.execute(c, splitted); //Don't really care about the return value. ;D
            } catch (Exception e) {
                c.getPlayer().dropMessage(6, "An error occurred while processing the command: " + e.getMessage());
                if (c.getPlayer().isGM()) {
                    c.getPlayer().dropMessage(6, "An error occurred while processing: " + e.getMessage());
		    e.printStackTrace();
        ...
        if (c.getPlayer().getGMLevel() > PlayerGMRank.NORMAL.getLevel()) {
			...
			ret = co.execute(c, splitted);
                        ...
                        c.getPlayer().dropMessage(6, "An error occurred while processing the command: " + x.getMessage());
		    } catch (Exception e) {
			...
			if (c.getPlayer().isGM()) {
			    ...
			    logCommandToDB(c.getPlayer(), line, "internlog");
                    ...
                    c.getPlayer().dropMessage(6, "Command " + splitted[0] + " does not have execute permission.");
                }
                return true;
            }
        }
